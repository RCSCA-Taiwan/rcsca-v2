...
-- Version: 20260830212721; name: rls_baseline
...
create policy "case needs via case access" on public.case_needs for select using (exists(select 1 from public.support_cases c where c.id = case_id and (c.owner_user_id = auth.uid() or c.assigned_to = auth.uid() or public.is_admin('case_manager'))));
