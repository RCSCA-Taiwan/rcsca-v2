-- Trusted mutations. Grant execute only as needed after staging verification.
...
declare
  p public.activity_participations;
begin
  ...
  if p_approved and not exists (
    select 1 from public.sharing_footprints f where f.source_type='activity_participation' and f.source_id=p_participation_id
  ) then
