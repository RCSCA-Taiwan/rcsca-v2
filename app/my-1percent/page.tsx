...
export default function MyOnePercent(){
 ...
 return <main className="myPage">
  ...
  <section className="myHero"><div className="myHeroInner"><div className="myIntro"><div className="eyebrow">MY 1% · PERSONAL WORLD</div><h1>我的 <OneMark/>，<br/>從這裡開始被看見。</h1><p>MY 1% 不是會員資料頁，而是你在 Cycle of Goodness 裡的位置：你走過什麼、連上什麼、下一步還可以往哪裡走。</p><div className="roleSwitch" aria-label="身份預覽">{(['visitor','partner','member'] as Role[]).map(r=><button key={r} className={role===r?'active':''} onClick={()=>setRole(r)}><small>{roleCopy[r].en}</small><b>{roleCopy[r].label}</b></button>)}</div></div>
   <div className={`digitalIdentityCard tier-${currentLevel||1}`}><div className="cardShine"></div><div className="cardTop"><span>RCSCA · MY 1%</span><OneMark/></div><div className="cardRole"><small>{active.en}</small><strong>{active.label}</strong></div>{role==='visitor'?<div className="visitorCardText">尚未建立 MY 1%<br/><span>註冊後，這張卡才開始留下你的共享足跡。</span></div>:<><div className="cardName">王○○</div><div className="cardMeta">Lv.4 共享連結者 · 暖陽小隊</div><div className="cardMeta">同行第 2,846 天</div><div className="cardQr" aria-label="QR placeholder"><i></i><i></i><i></i><i></i></div></>}<div className="cardFooter">卡片材質隨參與階段進化；身份權限與 Level 分開計算。</div></div>
  </div></section>
  ...
  <section className="memberLayer"><div className="myWrap"><div className="memberLayerHead"><div><div className="eyebrow">RCSCA MEMBER · DEEPER LAYER</div><h2>不是更高等級，而是多一層共享網絡。</h2><p>一般會員與永久會員前台都只顯示 RCSCA MEMBER；相同行為維持相同 XP，不用會費買等級。</p></div><div className="memberSeal">RCSCA<br/><b>MEMBER</b></div></div>
